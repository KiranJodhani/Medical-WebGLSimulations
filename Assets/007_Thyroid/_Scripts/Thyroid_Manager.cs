...
public class Thyroid_Manager : MonoBehaviour
{
    public static Thyroid_Manager Instance;

    [Header(" ##### FNA INSTANCE ####")]
    public Thyroid_Data[] Thyroid_Data_Instance;
    ...
    IEnumerator AnimateMenuButton()
    {
        ...
        s.SetLoops(-1);
        yield return new WaitForSeconds(Thyroid_Data_Instance[ScreenIndex].Item_Audio[Simulation_Backend.SelectedLanguageID].length+1);
        s.Kill();
    ...
    {
        if(!FNA_Adapter_Coll.GetComponent<Thyroid_Element>().MyHand.activeSelf)
        {
    ...
    {
        DOTween.KillAll();
        SceneManager.LoadScene("Thyroid");
    }
    ...
    {
        FNA_AudioSource.Stop();
        StepDescription.text = Thyroid_Data_Instance[ScreenIndex].Step_Description[Simulation_Backend.SelectedLanguageID];
        FNA_AudioSource.clip = Thyroid_Data_Instance[ScreenIndex].Item_Audio[Simulation_Backend.SelectedLanguageID];
    ...
    public void OnClickFNA_Item(GameObject Item)
    {
        ...
        if(Item.name== "FNA_Adapter")
        {
            ...
            {
                FNA_Adapter_Coll.transform.GetComponent<Thyroid_Element>().MyHand.transform.localPosition =
                    FNA_Adapter_Coll.transform.GetComponent<Thyroid_Element>().HandInitialPosition;
                Sequence s1 = DOTween.Sequence();
            ...
            else if (ScreenIndex == 6)
            {
                ...
                //NeedleMesh.GetComponent<Outline>().enabled = true;
                s1.Append(FNA_Adapter_Coll.transform.DOLocalMove(FNA_At_Step7.localPosition, 2f)).SetEase(Ease.Linear)
                    .OnComplete(OnFNASetAboveBreast);
                FNA_Step6_Arrow.SetActive(false);
                Sequence s3 = DOTween.Sequence();
                s3.Append(CameraObj.transform.DOLocalMove(CameraObj_Step7.localPosition, 1f)).SetEase(Ease.Linear);
            }
        ...
        {
            Syringe_Coll.GetComponent<Thyroid_Element>().MyHand.SetActive(false);
            FNA_Adapter_Coll.GetComponent<Thyroid_Element>().MyHand.SetActive(false);
        ...
        else if (Item.name == "Cover")
        {
            ...
            {
                Cover.transform.GetComponent<Thyroid_Element>().MyHand2.SetActive(true);
                Sequence s = DOTween.Sequence();
    ...
    {
        Cover.transform.parent = null;
        Cover.GetComponent<Thyroid_Element>().MyHand2.SetActive(false);
    ...
    void OnAdapterPlacedOnTray()
    {
        ...
        FNA_Adapter_Coll.gameObject.GetComponent<Thyroid_Element>().RemoveHand();
        Syringe_Coll.gameObject.GetComponent<Thyroid_Element>().RemoveHand();
    ...
    void Start_Step7()
    {
        Full_Breast_HandStep6.GetComponent<Thyroid_Rub_Animation>().StopRubAnimation();
    ...
    void OnFNA_StandFinished()
    {
        ...
        Sequence s1 = DOTween.Sequence();
        Syringe_Coll.GetComponent<Thyroid_Element>().MyHand.transform.localPosition = Syringe_Coll.GetComponent<Thyroid_Element>().HandInitialPosition;
        Syringe_Coll.GetComponent<Thyroid_Element>().MyHand.SetActive(true);
    ...
    {
        FNA_Adapter_Coll.GetComponent<Thyroid_Element>().RemoveHand();
    }
    ...
    void OnSyringeRemovedFromFNA()
    {
        ...
        Syringe_Coll.GetComponent<Thyroid_Element>().MyHand.SetActive(false);
        Syringe_Coll.GetComponent<Thyroid_Element>().MyHand2.SetActive(true);
