...
public class FNA_Manager : MonoBehaviour
{
    public static FNA_Manager Instance;

    [Header(" ##### FNA INSTANCE ####")]
    public FNAData[] FNAData_Instance;
    ...
    private void Awake()
    {
        if(SceneManager.GetActiveScene().name=="C_Spec")
        {
            Instance = this;
        }
    }
    ...
    IEnumerator AnimateMenuButton()
    {
        ...
        s.SetLoops(-1);
        yield return new WaitForSeconds(FNAData_Instance[ScreenIndex].Item_Audio[Simulation_Backend.SelectedLanguageID].length+1);
        s.Kill();
    ...
    {
        if(!FNA_Adapter_Coll.GetComponent<FNA_Element>().MyHand.activeSelf)
        {
    ...
    {
        FNA_AudioSource.Stop();
        StepDescription.text = FNAData_Instance[ScreenIndex].Step_Description[Simulation_Backend.SelectedLanguageID];
        FNA_AudioSource.clip = FNAData_Instance[ScreenIndex].Item_Audio[Simulation_Backend.SelectedLanguageID];
    ...
    public void OnClickFNA_Item(GameObject Item)
    {
        ...
        if(Item.name== "FNA_Adapter")
        {
            ...
            {
                FNA_Adapter_Coll.transform.GetComponent<FNA_Element>().MyHand.transform.localPosition =
                    FNA_Adapter_Coll.transform.GetComponent<FNA_Element>().HandInitialPosition;
                Sequence s1 = DOTween.Sequence();
        ...
        {
            Syringe_Coll.GetComponent<FNA_Element>().MyHand.SetActive(false);
            FNA_Adapter_Coll.GetComponent<FNA_Element>().MyHand.SetActive(false);
        ...
        else if (Item.name == "Cover")
        {
            ...
            {
                Cover.transform.GetComponent<FNA_Element>().MyHand2.SetActive(true);
                Sequence s = DOTween.Sequence();
    ...
    {
        Cover.transform.parent = null;
        Cover.GetComponent<FNA_Element>().MyHand2.SetActive(false);
    ...
    void OnAdapterPlacedOnTray()
    {
        ...
        FNA_Adapter_Coll.gameObject.GetComponent<FNA_Element>().RemoveHand();
        Syringe_Coll.gameObject.GetComponent<FNA_Element>().RemoveHand();
    ...
    void Start_Step7()
    {
        Full_Breast_HandStep6.GetComponent<Rub_Animation>().StopRubAnimation();
    ...
    void OnFNA_StandFinished()
    {
        ...
        Sequence s1 = DOTween.Sequence();
        Syringe_Coll.GetComponent<FNA_Element>().MyHand.transform.localPosition = Syringe_Coll.GetComponent<FNA_Element>().HandInitialPosition;
        Syringe_Coll.GetComponent<FNA_Element>().MyHand.SetActive(true);
    ...
    {
        FNA_Adapter_Coll.GetComponent<FNA_Element>().RemoveHand();
    }
    ...
    void OnSyringeRemovedFromFNA()
    {
        ...
        Syringe_Coll.GetComponent<FNA_Element>().MyHand.SetActive(false);
        Syringe_Coll.GetComponent<FNA_Element>().MyHand2.SetActive(true);
